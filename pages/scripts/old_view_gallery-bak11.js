...
function lookInto(param) {
    for (var str in param) {
        console.log(str, param[str]);
    }
}
...
function init() {
    ...
    baseTextTo16px = Math.max(baseTextTo10PxTo10Percent * 1.601, 16); //1.601 @1000px, comfy≈16 (alt9463)
    ...
    stage.update();
    //console.log("baseTextSizeFromDims.getBounds()",baseTextSizeFromDims.getBounds());
...
function reportClick() {
    ...
    for (var i = 0; i < 10; i++) {
        ...
        console.log("text.height: ", text.textHeight);
        text.y = i * text.textHeight;
...
function getNumberResized(num) {
    //  var temp = parseFloat(num * (baseTextTo10Px / 10) * 100);
    var temp = parseFloat(num * ((baseTextVariableTen * 0.74) / 10) * 100);
    //  var temp = parseFloat(num * (baseTextVariableTen ) * 100);
...
function stageSetupNowStart() {
     ...
     */
    window.addEventListener("resize", () => {
        // a test using a check can be handled like this:
        //   if (checkbox.checked) {
        //   resizeObserver.observe(divElem);
        // } else {
        //   resizeObserver.unobserve(divElem);
