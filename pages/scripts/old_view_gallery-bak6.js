...
function init() {
    ...
    //baseTextTo10PxThenPercent was getting the number 16 (for the size of text?) to equal 10px wide.
    //      baseTextTo10PxThenPercent = parseFloat(16 * scaleFactor) * 0.25 * 16 || 16;
    //with the above math, using the font family "arial", the text was
    //       10 pixels tall, but the numbers show the text was at
    //       only "7.22216796875". The font wasn't scaled to a percentage of the view.

    //this is scaled to 10px based on the displayArea.width 1000
    //baseTextTo10PxThenPercent = (w * 0.01);
    //the text is passed an undefined at this time. Not sure how that works, so it's the
...
function reportClick() {
    ...
    console.log("testdimsTextSize: ", baseTextTo10PxThenPercent);
    //stage.update();
}
