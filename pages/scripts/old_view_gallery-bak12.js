...
function reportScaled(event) {
    console.log("███└└say:reportScaled", event);
    simpleGalleryConfig.isResizing = false;
}
...
function stageSetupNowStart() {
    ...
    simpleGalleryConfig.stage.addEventListener("stagemousedown", reportClick);
...
function tick(event) {
    simpleGalleryConfig._mainStage.update(event);
}
...
function performResizeDuties() {
    ...
    // GameData.globalScale;  //-- probably not a bad idea.
    // GameData.defaultTextFormat
    // GameData.defaultLineStyles
}

function layoutGui() {
    console.log("layoutGui");
}
...
function preloadStuff() {
    ...
    simpleGalleryConfig._preLoader.addEventListener("complete", function () {
        console.log("preload is NOW COMPLETED.");
        var attractionAnim = new createjs.Sprite(
            simpleGalleryConfig._preLoader.getResult("woody")
        );
        //attractionAnim.scale = 0.333;
        attractionAnim.play();
        ...
        simpleGalleryConfig._mainStage.addChild(anMC);
        anMC.x = simpleGalleryConfig._mainStage.getBounds().width / 2;
        anMC.y = simpleGalleryConfig._mainStage.getBounds().height / 2;
    });
    ...
    simpleGalleryConfig._preLoader.loadManifest(animatedPreloader, true);
...
function preloadProgress(e) {
    ...
    var loadTextH = preloadingText.getMeasuredHeight();
...
function prepPreloader() {
    ...
    simpleGalleryConfig._mainStage.enableMouseOver();
    aspect =
        Math.min(displayArea.height, displayArea.width) /
        Math.max(displayArea.height, displayArea.width);
    // grab canvas width and height for later calculations:
    ...
    w = displayArea.width;
    h = displayArea.height;
    simpleGalleryConfig._mainStage.setBounds(0, 0, w, h);

    simpleGalleryConfig._mainStage.canvas.width =
        simpleGalleryConfig._mainStage.getBounds().width;
    simpleGalleryConfig._mainStage.canvas.height =
        simpleGalleryConfig._mainStage.getBounds().height;
    ...
    bg.graphics.beginFill("#BADA55").drawRect(0, 0, w, h).endFill();
    bgMC.setBounds(0, 0, w, h);
    simpleGalleryConfig._mainStage.addChild(bgMC);
    ...
    bgMC.addChild(subject);
    subject.cursor = "pointer";
    simpleGalleryConfig._mainStage.update();
...
function initSimpleGallery() {
    ...
    console.log(simpleGalleryConfig);
...
Object.defineProperty(SimpleGalleryConfig.prototype, "isResizing", {
    get: function () {
        return isResizing;
    },
    set: function (param) {
